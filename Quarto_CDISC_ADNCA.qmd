...
::: notes
Non-compartmental analysis looks at the concentration of a drug in the body (e.g. blood) over time
:::
...
Note a relative time variable may refer to previous dose or next upcoming dose

::: notes
Here is a list of important nominal and actual relative time variables

Timing Variables may refer to previous dose or next upcoming dose
:::
...
-   [Calculate Baseline and Change from Baseline](#base)
...
Note that `NFRLT` is expanded by 24 hours for each record
...
Note `ADTM_prev` is the Datetime of the Previous Dose `AVISIT_prev` is the Analysis Visit of the Previous Dose
...
::: notes
Similarly we find the next upcoming dose Note ADTM_next is the datetime for the next dose
:::
...
Note `ADTM_next` is the Datetime of the Next Upcoming Dose
...
Note `NFRLT_prev` is the Nominal Relative Time from the Previous Dose `NFRLT_next` is the Nominal Relative Time for the Next Dose
...
Note `AFRLT` Actual Relative Time from Analyte First Dose `ARRLT` Actual Relative Time from Reference Dose
...
Note `NFRLT` Nominal Relative Time from Analyte First Dose `NRRLT` Nominal Relative Time from Reference Dose
...
Note `ATPT` Analysis Timepoint `ATPTREF` Analysis Timepoint Reference `AVAL` Analysis Value
...
Note that `NRRLT` and `ARRLT` equal zero for next upcoming dose

::: notes
Here output for duplicated records ATPT is changed to "Pre-dose" Note `DTYPE` = "COPY" Note that `NRRLT` and `ARRLT` equal zero for next upcoming dose
:::
...
-   Slack Pharmaverse [Channel](https://pharmaverse.slack.com/ssb/redirect){target="_blank"}
